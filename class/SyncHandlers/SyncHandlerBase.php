  ...
  public static function sync_many(array $shortcodes)
  {
    ...
    foreach ($shortcodes as $shortcode) {
      ...
      if (!empty($data)) {
        ...
        write_log('Error fetching data for shortcode: ' . $shortcode, !$options->turn_off_debug_logging);
      } 
    }
  }
        ...
        foreach($tag_data as $tag_name => $tag_info){
          ...
          if ($existing_term === false) {
            ...
            if (!is_wp_error($term)) {
              ...
              write_log('Error creating tag: ' . $term->get_error_message(), !$options->turn_off_debug_logging);
            }
          } else {
      ...
      foreach ($taxonomies as $taxonomy) {
        ...
        foreach ($postarr_arrays["post_tag_group_tags"] as $tag) {
          ...
          if (is_wp_error($result)) {
            write_log("Failed to assign term '{$term->name}' to taxonomy '{$taxonomy->name}' on post ID {$post_id}: {$result->get_error_message()}", !$options->turn_off_debug_logging);
          }
        }
      }
    }
